...
namespace KubectlParser
{
    public static class Exporter
    {
        ...
        public static void CreateSettings(string root, ImmutableArray<string> previousVerbs, Command command)
        {
            ...
            builder.AppendLine();
            builder.AppendLine("namespace Cake.Kubectl");
            builder.AppendLine("{");
            builder.AppendLine("\t/// <summary>");
            builder.AppendLine($"\t/// {EncodeComment(command.Short)}");
            builder.AppendLine("\t///");
            ...
            builder.AppendLine($"\tpublic sealed class Kubectl{verbs}Settings : AutoToolSettings");
            builder.AppendLine("\t{");
            foreach (var option in command.Options)
            {
                ...
                builder.AppendLine("\t\t///");
                builder.AppendLine($"\t\t/// {EncodeComment(option.Description)}");
                builder.AppendLine("\t\t/// </summary>");
                builder.AppendLine($"\t\tpublic {option.TypeName} {option.PropertyName} {{ get; set; }}");
            }
            builder.AppendLine("\t}");
            builder.AppendLine("}");
            string filename = Path.Combine(root, $"Kubectl{verbs}Settings.cs");
        ...
        {
            string verbs = string.Join("", previousVerbs.Select(v => Command.PascalizeVerb(v)).ToImmutableArray().Add(command.PascalCaseVerb));
            string settingsTypeName = $"Kubectl{verbs}Settings";
            StringBuilder builder = new StringBuilder();
            ...
            builder.AppendLine("namespace Cake.Kubectl");
            builder.AppendLine("{");
            builder.AppendLine("\tpartial class KubectlAliases");
            builder.AppendLine("\t{");
            ...
            void AppendMethod(string? methodNamePostfix, string returnType, string runCommand, string? resultTypeMeta)
            {
                ...
                builder.AppendLine($"\t\tpublic static {returnType} Kubectl{verbs}{methodNamePostfix}(this ICakeContext context, {settingsTypeName} settings)");
                builder.AppendLine($"\t\t{{");
            ...
            builder.AppendLine("\t}");
            builder.AppendLine("}");
            string filename = Path.Combine(root, $"Kubectl.Alias.{verbs}.cs");
            if (File.Exists(filename))
            {
                File.Delete(filename);
            }
        ...
        public static string[] TrimLines(string[] lines)
        {
            ...
            {
                return lines.Take(lastLine.Value + 1)
                    .Select(l => EncodeComment(l))
                    .ToArray();
            }
            return new string[0];
        }

        internal static string EncodeComment(string line)
        {
