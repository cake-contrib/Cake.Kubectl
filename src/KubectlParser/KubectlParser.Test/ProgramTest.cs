...
namespace KubectlParser.Test
{
    public class ProgramTest
    {
        ...
        public class GetOptionFromText
        {
            ...
            {
                const string text = "      --allow-missing-template-keys=true: If true, ignore any errors in templates when a field or map key is missing in the template. Only applies to golang and jsonpath output formats.";

                var option = Program.GetOptionFromText(text);
            ...
            {
                const string text = "  -f, --filename=[]: Filename, directory, or URL to files to use to create the resource";
